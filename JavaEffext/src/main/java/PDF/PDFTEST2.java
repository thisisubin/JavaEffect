...
public class PDFTEST2 {
    public static void main(String[] args) throws IOException {
        ...
        new PDFTEST2().createPdf(dest); // PDFTest2객체 -> 메서드 createPdf 매개변수 dest
    ...
    public void createPdf(String dest) throws IOException {
        ...
        Document document = new Document(pdf, PageSize.A4); // A4사이즈

        //Font Set
        PdfFont headerFont = null;
        PdfFont bodyFont = null;

        headerFont = PdfFontFactory.createFont("NanumYaGeunHaNeunGimJuIm.ttf",
                "Identity-H");

        bodyFont = PdfFontFactory.createFont("NanumYaGeunHaNeunGimJuIm.ttf",
                "Identity-H");

        //실수 배열 생성 -> 테이블 생성
        float[] columnWidths = {1, 2, 2, 2, 2, 2}; //컬럼 넓이 비율
